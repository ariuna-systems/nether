        ...
        while not stop_event.is_set():
            try:
                ...
                if self.processed_count % 5 == 0:
                    await dispatch(
                        DataProcessed(processor_name="TemperatureProcessor", processed_count=self.processed_count)
                    )
        ...
        while not stop_event.is_set():
            try:
                ...
                if self.processed_count % 3 == 0:
                    await dispatch(
                        DataProcessed(processor_name="HumidityProcessor", processed_count=self.processed_count)
                    )
