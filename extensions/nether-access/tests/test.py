...
class AsyncPostgresConnector:
  ...
  async def transaction(self):
    conn = await psycopg.AsyncConnection.connect(
      **self._credentials, row_factory=psycopg.rows.dict_row, autocommit=True
    )
    try:
      cursor = conn.cursor()
      yield cursor
...
async def connector():
  ...
  connector = AsyncPostgresConnector(
    credentials={"host": "localhost", "dbname": "postgres", "user": "postgres", "password": "postgres"}
  )
  ...
  db_connector = AsyncPostgresConnector(
    credentials={"host": "localhost", "dbname": test_db_name, "user": "postgres", "password": "postgres"}
  )
...
async def test_access(connector):
  ...
  account_secret = await account_service._create_account(
    account_name=account_name, email="test@test", password_hash=password_hash, role_ids=[]
  )
  assert (account := await account_repository.search_by_name(account_name)) is not None

  session_id = await access_service._validate_account(
    account_name=account_name, password_hash=password_hash, email=None
  )
  jwt_token = await access_service._validate_one_time_password(
    account_session_id=session_id, one_time_password=pyotp.TOTP(account_secret).now()
  )
